...
export class TimeZoneEffects {
  ...
  @Effect()
  fetchTimeZone$ = this.actions$.ofType(fromTimeZoneActions.TIME_ZONE_REQUEST).switchMap(action => {
    const userId: number = action.payload;
    return this.http
      .get(`/api/users/${userId}/time-zones`, {
  ...
  @Effect()
  requestTimeZoneUser$ = this.actions$
    ...
    .switchMap(actionWithStore => {
      ...
      if (!!latestStore.session.userInfo && latestStore.session.userInfo.id === requestedUserId) {
        ...
        return Observable.of(fromTimeZoneActions.requestTimeZoneUserSuccess(latestStore.session.userInfo));
      }
      return this.http
  ...
  @Effect()
  createTimeZone$ = this.actions$.ofType(fromTimeZoneActions.CREATE_TIME_ZONE).switchMap(action =>
    this.http
      .post(`/api/users/${action.payload.userId}/time-zones`, action.payload.timeZone, { headers: SessionService.getSessionHeader() })
      .map(res => {
        if (res.ok) {
  ...
  @Effect()
  updateTimeZone$ = this.actions$.ofType(fromTimeZoneActions.UPDATE_TIME_ZONE).switchMap(action =>
    this.http
      .put(`/api/time-zones/${action.payload.timeZone.id}`, action.payload.timeZone, { headers: SessionService.getSessionHeader() })
      .map(res => {
        if (res.ok) {
  ...
  @Effect()
  deleteTimeZone$ = this.actions$.ofType(fromTimeZoneActions.DELETE_TIME_ZONE).switchMap(action =>
    this.http
      .delete(`/api/time-zones/${action.payload.id}`, {
        headers: SessionService.getSessionHeader()
      })
      .map(res => {
        if (res.ok) {
