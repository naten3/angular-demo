...
export class TimeZoneItemComponent implements OnInit {
  ...
  timeZone: TimeZone;
  @Output()
  updateTimeZoneEmitter = new EventEmitter<TimeZone>();
  @Output()
  ...
  getGmtOffest(): string {
    return (this.timeZone.positiveOffset ? '+' : '-') + this.timeZone.offsetHours + ':' + padStart(this.timeZone.offsetMinutes, 2, '0');
  }

  ngOnInit() {
    const timeTimer$ = timer(0, 10000);
    this.timeInZone$ = timeTimer$.map(unused => {
      const currentTime = moment.utc();
      const offset = {
        hours: this.timeZone.offsetHours,
        minutes: this.timeZone.offsetMinutes
      };
      const timeWithOffset = this.timeZone.positiveOffset ? currentTime.add(offset) : currentTime.subtract(offset);
